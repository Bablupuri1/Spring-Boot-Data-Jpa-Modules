...
public class UserService {
	...
	/**
	 * Create a new user only if mobile number is not already registered. If mobile
	 * is already registered, send email with existing credentials.
	 */
	public ApiResponse createUser(UserPayload userPayload) {
		try {
			// Validate if payload is null
			if (userPayload == null) {
				return new ApiResponse(false, "User object is null", HttpStatus.SC_BAD_REQUEST, userPayload, res);
			}
			...
			if (userPayload.getMobile() == null || userPayload.getMobile().trim().isEmpty()) {
				return new ApiResponse(false, "User mobile number is null or empty", HttpStatus.SC_BAD_REQUEST,
						userPayload, res);
			}
			...
			if (userRepository.existsByMobile(userPayload.getMobile())) {
				...
				res = emailservice.sendUserCredentials(existingUser.getEmail(), existingUser.getName(),
						existingUser.getPassword());

				// Return response - duplicate mobile
				return new ApiResponse(false, "User already exists. Credentials resent to email.",
						HttpStatus.SC_CONFLICT, existingUser, res);
			} else {
				...
				res = emailservice.sendUserCredentials(savedUser.getEmail(), savedUser.getName(),
						savedUser.getPassword());

				// Return success response
				return new ApiResponse(true, "User registered successfully", HttpStatus.SC_OK, savedUser, res);
			}
			...
			return new ApiResponse(false, "Server error", HttpStatus.SC_INTERNAL_SERVER_ERROR, e.getMessage(), res);
		}
	}
	...
	public ApiResponse Authenticate(String username, String password) {
	    ...
	    if (user != null) {
	        System.out.println("UserService.Authenticate() matched success");

	        return new ApiResponse(true, "Login Successfully....", 200, user, null);
	    } else {
	        System.out.println("UserService.Authenticate() did not match...");
	        return new ApiResponse(false, "Username Or Password Mismatch", 409, null, null);
	    }
	}
